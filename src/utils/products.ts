export const products = [
  {
    name: "Açafrão",
    price: 5.5,
    picture: "/assets/products/acafrao.png",
    description:
      "O açafrão é extraído dos estigmas da flor Crocus sativus e possui propriedades antioxidantes e anti-inflamatórias, sendo utilizado em diversos pratos como arroz, risotos, sopas e até mesmo em doces.",
  },
  {
    name: "Alho Granulado",
    price: 5.5,
    picture: "/assets/products/alho-granulado.png",
    description:
      "O alho granulado é um tempero muito utilizado na culinária, com sabor e aroma intensos. É obtido a partir da moagem do alho fresco e pode ser utilizado em diversas preparações, como molhos, assados, refogados, massas e saladas. Além disso, o alho possui propriedades medicinais, sendo considerado um poderoso antioxidante e anti-inflamatório.",
  },
  {
    name: "Bicarbonato de Sódio",
    price: 5.0,
    picture: "/assets/products/bicarbonato-de-sodio.png",
    description:
      "O bicarbonato de sódio é um composto químico versátil com muitas aplicações na culinária e produtos de limpeza. É um agente fermentador usado em receitas de pães e bolos, e um ingrediente comum em produtos de limpeza devido às suas propriedades de neutralização de odores e capacidade de limpeza profunda. Disponível em nossa loja em alta qualidade.",
  },
  {
    name: "Boldo do Chile",
    price: 4.5,
    picture: "/assets/products/boldo-do-chile.png",
    description:
      "O Boldo do Chile é uma planta originária da América do Sul e é conhecida por suas propriedades medicinais. O chá de Boldo é popularmente usado para aliviar problemas digestivos, como azia, gases e indigestão, além de auxiliar na desintoxicação do fígado. A planta também é usada em produtos de beleza e cuidados pessoais, como xampus e cremes para cabelos oleosos. Adquira agora mesmo o Boldo do Chile de alta qualidade em nossa loja!",
  },
  {
    name: "Capim Santo",
    price: 4.5,
    picture: "/assets/products/capim-santo.png",
    description:
      "O Capim Santo é uma planta herbácea comumente encontrada na América Latina e é conhecida por suas propriedades medicinais e culinárias. Suas folhas possuem um aroma cítrico e são utilizadas como tempero em diversas receitas, como chás, sucos, sopas e pratos de frango e peixe. Além disso, o Capim Santo é valorizado por suas propriedades medicinais, sendo utilizado para aliviar sintomas de ansiedade, estresse e problemas digestivos.",
  },
  {
    name: "Cebola Desidratada",
    price: 5.5,
    picture: "/assets/products/cebola-desidratada.png",
    description:
      "A Cebola Desidratada é um ingrediente culinário versátil e conveniente, que oferece o sabor e aroma característicos da cebola em diversas preparações culinárias. Ela é obtida a partir do processo de desidratação da cebola fresca, e pode ser utilizada em sopas, molhos, saladas, guisados e outros pratos que exigem o sabor da cebola. Além disso, a cebola é rica em nutrientes e compostos antioxidantes, que são benéficos para a saúde.",
  },
  {
    name: "Chá Verde",
    price: 4.5,
    picture: "/assets/products/cha-verde.png",
    description:
      "O Chá Verde é uma bebida popular em todo o mundo, conhecida por seus benefícios à saúde. Ele é feito a partir das folhas da planta Camellia sinensis, que é rica em antioxidantes e outros compostos benéficos para o organismo. O Chá Verde pode ajudar na redução do risco de doenças cardiovasculares, fortalecimento do sistema imunológico, melhora da função cerebral, redução do estresse e ansiedade.",
  },
  {
    name: "Chimichurri",
    price: 5.5,
    picture: "/assets/products/chimichurri.png",
    description:
      "O Chimichurri é um molho de tempero originário da Argentina e Uruguai, usado para realçar o sabor de carnes grelhadas, churrasco e outros pratos. É feito com ervas frescas, como salsa, orégano e manjericão, alho, vinagre e azeite de oliva. O Chimichurri é um condimento versátil e saboroso, que pode ser usado como marinada ou para adicionar sabor a qualquer prato.",
  },
  {
    name: "Colorau",
    price: 5.5,
    picture: "/assets/products/colorau.png",
    description:
      "O colorau é um tempero em pó bastante utilizado na culinária brasileira, especialmente em pratos como o arroz, o feijão e a carne. Com sua cor vibrante e sabor característico, ele é capaz de realçar o sabor de diversos alimentos, conferindo um toque especial às suas receitas.",
  },
  {
    name: "Curry",
    price: 5.5,
    picture: "/assets/products/curry.png",
    description:
      "O curry é um blend de temperos originário da culinária indiana, que combina diversas especiarias como cúrcuma, cominho, coentro, gengibre e pimenta, entre outras. Ele é usado para temperar pratos de carne, legumes e arroz, e seu sabor pode variar de suave a extremamente picante.",
  },
  {
    name: "Dry Rub",
    price: 5.5,
    picture: "/assets/products/dry-rub.png",
    description:
      "Dry Rub é uma mistura de especiarias e temperos secos que são esfregados na carne antes de serem grelhados, assados ou defumados. Ele é utilizado para dar sabor e textura à carne, criando uma crosta saborosa e caramelizada na superfície. Em nossa loja, oferecemos diferentes tipos de Dry Rub, cada um com uma combinação única de especiarias e temperos, que podem variar desde sabores mais suaves até mais picantes.",
  },
  {
    name: "Feijão Supremo",
    price: 5.5,
    picture: "/assets/products/feijao-supremo.png",
    description:
      "Este tempero específico para feijão é uma mistura de especiarias e ingredientes que são frequentemente usados na culinária brasileira para dar sabor aos feijões. Ele contém cebola e alho para fornecer um sabor salgado e aromático, além de louro em pó para adicionar uma nota de sabor terroso. O caldo de bacon e o caldo de costela são adicionados para dar um sabor rico e defumado, enquanto a páprica defumada dá um sabor adicional de fumaça e um pouco de calor.",
  },
  {
    name: "Lemon Ervas",
    price: 5.5,
    picture: "/assets/products/lemon-ervas.png",
    description:
      "Lemon Ervas é um tempero em pó que combina o sabor cítrico do limão com um mix de ervas aromáticas, como alecrim, tomilho e manjericão. Ele é ideal para temperar carnes, peixes, saladas e legumes, conferindo um sabor fresco e delicado aos pratos. O Lemon Ervas é produzido com ingredientes de alta qualidade e cuidadosamente selecionados, garantindo um sabor autêntico e marcante em suas receitas. ",
  },
  {
    name: "Lemon Pepper",
    price: 5.5,
    picture: "/assets/products/lemon-pepper.png",
    description:
      "Lemon Pepper é uma mistura de especiarias e temperos que combina o sabor cítrico do limão com a picância da pimenta preta. Ele é utilizado para temperar carnes, aves, peixes e saladas, conferindo um sabor marcante e um aroma fresco aos pratos. O Lemon Pepper é produzido com ingredientes de alta qualidade e cuidadosamente selecionados, garantindo um sabor autêntico e marcante em suas receitas.",
  },
  {
    name: "Oregano",
    price: 5.5,
    picture: "/assets/products/oregano.png",
    description:
      "O orégano é uma erva aromática muito utilizada na culinária mediterrânea e latino-americana, especialmente em pratos com tomate, carne, frango, peixe e massas. Ele tem um sabor característico e levemente amargo, que pode variar de acordo com a região de cultivo e as condições climáticas. O orégano é uma fonte rica de antioxidantes e possui propriedades anti-inflamatórias, podendo trazer benefícios para a saúde.",
  },
  {
    name: "Páprica Defumada",
    price: 5.5,
    picture: "/assets/products/paprica-defumada.png",
    description:
      "A páprica defumada é um tempero em pó feito a partir de pimentões secos que foram defumados em fogo de lenha antes de serem moídos. Ele é muito utilizado na culinária espanhola e húngara para temperar carnes, aves, peixes, legumes e arroz, conferindo um sabor defumado e levemente adocicado aos pratos. A páprica defumada é uma excelente fonte de vitamina C e antioxidantes, e pode trazer benefícios para a saúde.",
  },
  {
    name: "Pimenta do Reino",
    price: 5.5,
    picture: "/assets/products/pimenta-do-reino.png",
    description:
      "A pimenta do reino é uma especiaria muito utilizada na culinária de todo o mundo, especialmente para temperar carnes, aves, peixes, molhos e sopas. É produzida a partir dos frutos da planta Piper nigrum, que são colhidos quando ainda verdes e secos ao sol até que fiquem pretos e enrugados. Possui um sabor picante e aromático, e é uma excelente fonte de antioxidantes e nutrientes como manganês e vitamina K.",
  },
  {
    name: "Sal Rosa do Himaláia",
    price: 5.5,
    picture: "/assets/products/sal-rosa-do-himalaia.png",
    description:
      "O Sal Rosa do Himalaia é um tipo de sal que é extraído de depósitos antigos nas montanhas do Himalaia, na Ásia. Ele é conhecido por sua cor rosa suave e sua textura crocante, além de seu sabor distinto e levemente adocicado. O Sal Rosa do Himalaia é uma excelente fonte de minerais e oligoelementos, incluindo cálcio, magnésio e potássio, e é considerado uma opção mais saudável do que o sal de mesa convencional. ",
  },
  {
    name: "Tempero Baiano",
    price: 5.5,
    picture: "/assets/products/tempero-baiano.png",
    description:
      "A páprica doce é um tempero em pó feito a partir de pimentões vermelhos secos e moídos. É um ingrediente comum na culinária húngara e espanhola, e tem um sabor suave e levemente adocicado. A páprica doce é frequentemente usada para adicionar cor e sabor a pratos como sopas, ensopados, carnes, frango e frutos do mar. É uma excelente opção para quem prefere sabores mais suaves e adocicados em suas receitas.",
  },
  {
    name: "Páprica Picante",
    price: 5.5,
    picture: "/assets/products/paprica-picante.png",
    description:
      "A páprica picante é um tempero em pó feito a partir de pimentões vermelhos secos e moídos, com um sabor forte e ardente. É um ingrediente comum na culinária húngara e espanhola, e é ideal para quem gosta de sabores picantes em suas receitas. A páprica picante é frequentemente usada para dar um toque de calor a pratos como sopas, ensopados, carnes, frango e frutos do mar. Seu sabor ardente pode variar de moderado a intenso, dependendo da marca e da quantidade utilizada na receita.",
  },
  {
    name: "Arroz Supremo",
    price: 5.5,
    picture: "/assets/products/arroz-supremo.png",
    description:
      "O tempero Arroz Supremo é uma mistura de especiarias que contém alho granulado, cebola desidratada, açafrão, orégano e manjericão. É um tempero versátil que pode ser usado para dar sabor e aroma ao arroz e outros pratos, como massas, sopas e legumes. O alho e a cebola adicionam um sabor delicioso e característico, enquanto o açafrão dá cor ao prato. O orégano e o manjericão adicionam um toque aromático, tornando o tempero Arroz Supremo uma escolha popular para quem busca um sabor intenso e delicioso em suas receitas.",
  },
  {
    name: "Pimenta Calabresa",
    price: 5.5,
    picture: "/assets/products/pimenta-calabresa.png",
    description:
      "A pimenta calabresa é um tempero em  pimentas secas, geralmente originárias da região da Calábria, na Itália. É um tempero popular na culinária italiana e brasileira, conhecido por seu sabor forte e picante. A pimenta calabresa é frequentemente usada para dar um toque de calor a pratos como pizzas, massas, molhos, carnes e frutos do mar. O nível de ardência da pimenta calabresa pode variar, mas geralmente é mais picante do que a pimenta-do-reino. É uma excelente opção para quem busca um tempero picante e saboroso em suas receitas.",
  },
  {
    name: "Ana Maria",
    price: 5.5,
    picture: "/assets/products/ana-maria.png",
    description:
      "O tempero Ana Maria é uma mistura de especiarias utilizada para dar sabor e aroma a pratos diversos da culinária brasileira. Ele é composto por ingredientes como alho, cebola, pimenta-do-reino, colorau, cominho, louro, coentro e salsa. É um tempero bastante versátil, que pode ser utilizado em diversos tipos de pratos, como carnes, frangos, peixes, legumes, arroz e feijão. O tempero Ana Maria é muito popular no Brasil, e é conhecido por seu sabor intenso e delicioso, que ajuda a realçar o sabor dos alimentos.",
  },
  {
    name: "Edu Guedes",
    price: 5.5,
    picture: "/assets/products/edu-guedes.png",
    description:
      "O tempero Edu Guedes é uma mistura de especiarias utilizada para dar sabor e aroma a pratos diversos da culinária brasileira. Ele é composto por ingredientes como alho, cebola, orégano, salsa, tomilho, manjericão, louro e pimenta-do-reino. É um tempero bastante versátil, que pode ser utilizado em diversos tipos de pratos, como carnes, frangos, peixes, legumes, arroz e feijão. O tempero Edu Guedes é muito popular no Brasil, especialmente entre aqueles que buscam dar um toque gourmet às suas receitas. É conhecido por seu sabor delicioso e sofisticado, que ajuda a realçar o sabor dos alimentos.",
  },
  {
    name: "Alho em Pó",
    price: 5.5,
    picture: "/assets/products/alho-em-po.png",
    description:
      "O alho em pó é um tempero utilizado para dar sabor e aroma a uma grande variedade de pratos. Ele é feito a partir de alho fresco que é descascado, desidratado e moído até se transformar em um pó fino. O alho em pó é uma opção prática para quem deseja adicionar o sabor do alho em suas receitas, sem precisar descascar e picar o alho fresco. Ele é frequentemente utilizado em pratos como carnes, aves, massas, sopas, molhos e legumes. O alho em pó é conhecido por seu sabor característico e intenso, que ajuda a realçar o sabor dos alimentos.",
  },
  {
    name: "Cebola, Alho e Salsa",
    price: 5.5,
    picture: "/assets/products/cebola-alho-salsa.png",
    description:
      "O tempero cebola, alho e salsa é uma mistura de especiarias utilizada para dar sabor e aroma a uma grande variedade de pratos. Ele é composto por ingredientes como cebola desidratada, alho em pó e salsa desidratada. É um tempero bastante versátil, que pode ser utilizado em diversos tipos de pratos, como carnes, frangos, peixes, legumes, arroz e feijão. O tempero cebola, alho e salsa é conhecido por seu sabor delicioso e equilibrado, que ajuda a realçar o sabor dos alimentos. É uma excelente opção para quem busca praticidade e sabor em suas receitas.",
  },
  {
    name: "Frango Supremo",
    price: 5.5,
    picture: "/assets/products/frango-supremo.png",
    description:
      "O tempero Frango Supremo é uma mistura de especiarias desenvolvida especialmente para dar sabor e aroma a pratos com frango. Ele é composto por ingredientes como cebola e alho desidratados, salsa, manjericão, açafrão, colorau e caldo de galinha. É um tempero bastante versátil, que pode ser utilizado em diversas receitas com frango, como ensopados, assados, grelhados e cozidos. O tempero Frango Supremo é conhecido por seu sabor equilibrado e delicioso, que ajuda a realçar o sabor do frango, deixando-o mais suculento e saboroso. É uma excelente opção para quem busca praticidade e sabor em suas receitas.",
  },
  {
    name: "Pimenta-do-reino Moída",
    price: 5.5,
    picture: "/assets/products/pimenta-do-reino-moida.png",
    description:
      "A pimenta-do-reino moída é um tempero muito utilizado na culinária para dar sabor e aroma a uma grande variedade de pratos. Ela é feita a partir dos grãos da pimenta-do-reino, que são moídos até se transformarem em um pó fino. A pimenta-do-reino moída é frequentemente utilizada em pratos como carnes, aves, peixes, massas, sopas e molhos. Ela é conhecida por seu sabor forte e picante, que ajuda a realçar o sabor dos alimentos, além de possuir propriedades digestivas e anti-inflamatórias. A pimenta-do-reino moída é um tempero básico em qualquer cozinha, pois é capaz de dar um toque especial e único a qualquer prato.",
  },
  {
    name: "Cominho Moído",
    price: 5.5,
    picture: "/assets/products/cominho-moido.png",
    description:
      "O cominho moído é um tempero muito utilizado na culinária para dar sabor e aroma a pratos salgados. Ele é feito a partir das sementes do cominho, que são secas e moídas até se transformarem em um pó fino. O cominho moído é frequentemente utilizado em pratos como carnes, aves, peixes, legumes, arroz e feijão. Ele é conhecido por seu sabor forte e levemente amargo, que ajuda a realçar o sabor dos alimentos, além de possuir propriedades digestivas e anti-inflamatórias. O cominho moído é um tempero muito versátil e pode ser utilizado em diversas receitas, conferindo um sabor único e especial aos pratos.",
  },
  {
    name: "Cominho em Grãos",
    price: 5.5,
    picture: "/assets/products/cominho-em-graos.png",
    description:
      "O cominho em grãos é um tempero utilizado na culinária para dar sabor e aroma a pratos salgados. Ele é feito a partir das sementes do cominho, que são secas e utilizadas inteiras. O cominho em grãos é frequentemente utilizado em pratos como carnes, aves, peixes, legumes, arroz e feijão. Ele é conhecido por seu sabor forte e levemente amargo, que ajuda a realçar o sabor dos alimentos, além de possuir propriedades digestivas e anti-inflamatórias. O cominho em grãos pode ser utilizado de diversas formas na culinária, como em infusões, marinadas, temperos e na finalização dos pratos. É um tempero muito versátil e pode conferir um sabor único e especial aos pratos.",
  },
  {
    name: "Coentro em Grãos",
    price: 5.5,
    picture: "/assets/products/coentro-em-graos.png",
    description:
      "O coentro em grãos é um tempero utilizado na culinária para dar sabor e aroma a pratos salgados. Ele é feito a partir das sementes da planta de coentro, que são secas e utilizadas inteiras. O coentro em grãos é frequentemente utilizado em pratos como carnes, aves, peixes, sopas e molhos. Ele é conhecido por seu sabor forte e levemente cítrico, que ajuda a realçar o sabor dos alimentos, além de possuir propriedades digestivas e anti-inflamatórias. O coentro em grãos pode ser utilizado de diversas formas na culinária, como em infusões, marinadas, temperos e na finalização dos pratos. É um tempero muito versátil e pode conferir um sabor único e especial aos pratos.",
  },
  {
    name: "Canela em Pó",
    price: 6.2,
    picture: "/assets/products/canela-em-po.png",
    description:
      "A canela em pó é um tempero muito utilizado na culinária para dar sabor e aroma a pratos doces e salgados. Ela é feita a partir da casca interna seca da árvore de canela, que é moída em pó. A canela em pó é conhecida por seu sabor doce e levemente picante, e é frequentemente utilizada em sobremesas, como bolos, tortas, pudins, mas também pode ser utilizada em pratos salgados, como carnes e molhos. Além disso, a canela em pó possui propriedades antioxidantes e anti-inflamatórias, que podem trazer benefícios à saúde. É um tempero muito versátil e pode conferir um sabor único e especial aos pratos.",
  },
  {
    name: "Louro",
    price: 4.5,
    picture: "/assets/products/folha-de-louro.png",
    description:
      "A folha de louro é uma erva aromática muito utilizada na culinária para dar sabor e aroma a pratos salgados, como ensopados, sopas, molhos e marinadas. Ela é obtida a partir da árvore do louro, nativa da região do Mediterrâneo. A folha de louro é rica em nutrientes e compostos antioxidantes, como os flavonoides e os ácidos fenólicos, que podem ajudar a prevenir doenças e melhorar a saúde em geral. Além disso, ela possui propriedades anti-inflamatórias e antissépticas, que podem ajudar a aliviar sintomas de problemas respiratórios e digestivos. A folha de louro é muito versátil e pode ser utilizada tanto fresca quanto seca, conferindo um sabor e aroma únicos aos pratos.",
  },
  {
    name: "Camomila",
    price: 4.5,
    picture: "/assets/products/camomila.png",
    description:
      "A camomila é uma planta medicinal conhecida por suas propriedades calmantes e relaxantes. Ela é nativa da Europa e da Ásia, e suas flores são utilizadas para fazer chás, infusões e extratos medicinais. A camomila é rica em compostos anti-inflamatórios e antioxidantes, como a apigenina e a quercetina, que podem ajudar a aliviar sintomas de ansiedade, estresse, insônia e dores de cabeça. Além disso, ela também pode ser utilizada para aliviar sintomas de problemas gastrointestinais, como cólicas, gases e diarreia. A camomila é uma opção natural e segura para quem busca relaxamento e bem-estar, e pode ser consumida tanto quente quanto fria.",
  },
  {
    name: "Alecrim",
    price: 5.0,
    picture: "/assets/products/alecrim.png",
    description:
      "O alecrim é uma erva aromática originária do Mediterrâneo, muito utilizada na culinária para dar sabor e aroma a diversos pratos, como carnes, legumes, pães e molhos. Além de seu uso culinário, o alecrim também é valorizado por suas propriedades medicinais, que incluem ação anti-inflamatória, antioxidante, antisséptica e estimulante do sistema nervoso. Ele é rico em compostos como o ácido rosmarínico, que ajuda a reduzir inflamações e prevenir danos celulares causados pelos radicais livres. Além disso, o alecrim também pode ajudar a melhorar a memória, a concentração e o humor, sendo uma opção natural para quem busca aumentar o bem-estar físico e mental.",
  },
  {
    name: "Hortelã",
    price: 5.0,
    picture: "/assets/products/hortela.png",
    description:
      "A hortelã é uma erva aromática muito utilizada na culinária e também valorizada por suas propriedades medicinais. Ela é conhecida por seu sabor refrescante e mentolado, que pode ser utilizado para dar sabor a chás, sobremesas, saladas e outros pratos. Além de seu uso culinário, o hortelã também é valorizado por suas propriedades digestivas, antiespasmódicas e analgésicas. Ela pode ajudar a aliviar sintomas de náuseas, vômitos, dores abdominais, cólicas menstruais e dores de cabeça. Além disso, o hortelã também pode ajudar a melhorar a respiração e a aliviar sintomas de alergias respiratórias, como a congestão nasal. O hortelã é uma opção natural e versátil para quem busca melhorar a saúde e o bem-estar.",
  },
  {
    name: "Hibisco",
    price: 5.5,
    picture: "/assets/products/hibisco.png",
    description:
      "O hibisco é uma flor de cor vibrante, muito utilizada na culinária e também valorizada por suas propriedades medicinais. Ele é comumente utilizado para preparar chás, sucos e sobremesas, e é conhecido por seu sabor levemente ácido e adocicado. Além de seu uso culinário, o hibisco é valorizado por suas propriedades antioxidantes, anti-inflamatórias e diuréticas. Ele pode ajudar a prevenir o envelhecimento precoce, proteger o coração e os vasos sanguíneos, além de auxiliar no controle da pressão arterial e na eliminação de toxinas do organismo. O hibisco é uma opção natural e saudável para quem busca melhorar a saúde e o bem-estar.",
  },
  {
    name: "Linhaça Dourada",
    price: 5.0,
    picture: "/assets/products/linhaca-dourada.png",
    description:
      "A linhaça dourada é uma semente oleaginosa muito valorizada por seus benefícios nutricionais. Ela é rica em ácidos graxos ômega-3, fibras, vitaminas e minerais. Além disso, contém lignanas, compostos fitoquímicos que possuem propriedades antioxidantes e anti-inflamatórias. A linhaça dourada é uma opção saudável para auxiliar na prevenção de doenças cardiovasculares, reduzir o colesterol, regular o intestino e melhorar a saúde da pele e dos cabelos. Ela pode ser consumida de diversas formas, como em preparações culinárias, adicionada a sucos e smoothies ou polvilhada sobre frutas e iogurtes. A linhaça dourada é uma excelente escolha para quem busca uma alimentação equilibrada e saudável.",
  },
  {
    name: "Linhaça Marrom",
    price: 5.0,
    picture: "/assets/products/linhaca-marrom.png",
    description:
      "A linhaça marrom é uma semente oleaginosa rica em nutrientes, semelhante à linhaça dourada. Ela é fonte de ácidos graxos ômega-3, fibras, vitaminas e minerais, além de lignanas, compostos fitoquímicos com propriedades antioxidantes e anti-inflamatórias. Assim como a linhaça dourada, a linhaça marrom pode ajudar na prevenção de doenças cardiovasculares, na redução do colesterol, no bom funcionamento do intestino e na melhora da saúde da pele e dos cabelos. A linhaça marrom pode ser adicionada a receitas culinárias, sucos, vitaminas e outros alimentos saudáveis. Seu consumo regular pode ajudar a promover uma alimentação equilibrada e contribuir para a manutenção da saúde.",
  },
  {
    name: "Spirulina",
    price: 12.0,
    picture: "/assets/products/spirulina.png",
    description:
      "A spirulina é uma microalga azul-esverdeada com alta concentração de nutrientes, incluindo proteínas, vitaminas e minerais. É uma fonte importante de antioxidantes e ácidos graxos essenciais, que podem ajudar na proteção contra doenças e no fortalecimento do sistema imunológico. Além disso, a spirulina contém clorofila, que pode ajudar a desintoxicar o organismo. Ela pode ser consumida em cápsulas, pó ou adicionada a smoothies, sucos e outras receitas culinárias saudáveis. A spirulina é uma ótima opção para vegetarianos, veganos e pessoas que buscam uma alimentação mais saudável e equilibrada.",
  },
  {
    name: "Semente de Abóbora",
    price: 8.7,
    picture: "/assets/products/semente-de-abobora.png",
    description:
      "As sementes de abóbora são uma excelente fonte de nutrientes, incluindo proteínas, gorduras saudáveis, fibras, vitaminas e minerais, como zinco e magnésio. São conhecidas por ajudar na regulação do açúcar no sangue, melhorar a saúde do coração e reduzir a inflamação. Além disso, a semente de abóbora pode ajudar na saúde da próstata e na prevenção de certos tipos de câncer. Ela pode ser consumida crua, torrada, adicionada a saladas, sopas, smoothies e outras receitas culinárias saudáveis. As sementes de abóbora são uma ótima opção para vegetarianos, veganos e pessoas que buscam uma alimentação mais saudável e equilibrada.",
  },
  {
    name: "Guaraná em Pó",
    price: 6.2,
    picture: "/assets/products/guarana-em-po.png",
    description:
      "O guaraná em pó é um ingrediente natural derivado das sementes da planta de guaraná, nativa da Amazônia. Ele é conhecido por sua alta concentração de cafeína, o que o torna um estimulante natural do sistema nervoso central. O consumo do guaraná em pó pode ajudar a melhorar o foco, a concentração e a energia, além de proporcionar benefícios para a saúde, como reduzir a fadiga, melhorar a função cerebral e aumentar a queima de gordura. O guaraná em pó pode ser adicionado a bebidas, smoothies, receitas culinárias e suplementos nutricionais, mas deve ser consumido com moderação, pois o excesso de cafeína pode causar efeitos colaterais indesejados, como ansiedade, insônia e irritabilidade.",
  },
  {
    name: "Maca Peruana",
    price: 9.0,
    picture: "/assets/products/maca-peruana.png",
    description:
      "A Maca Peruana é uma planta nativa do Peru, também conhecida como 'ginseng dos Andes'. É uma raiz comestível, rica em nutrientes como carboidratos, proteínas, fibras, vitaminas e minerais, além de compostos bioativos como alcaloides e glucosinolatos. A maca peruana é frequentemente utilizada como um superalimento que pode melhorar a saúde e o desempenho físico e mental. Alguns dos benefícios associados à sua ingestão incluem: aumento da energia, melhoria da libido, equilíbrio hormonal, redução do estresse e melhoria da função cognitiva. A maca peruana pode ser encontrada em pó, cápsulas ou extratos, e pode ser consumida em receitas culinárias ou como suplemento nutricional.",
  },
  {
    name: "Gengibre em Pó",
    price: 5.6,
    picture: "/assets/products/gengibre-em-po.png",
    description:
      "O gengibre em pó é uma especiaria produzida a partir da raiz do gengibre, que é seca e moída em um pó fino. É amplamente utilizado na culinária como tempero em pratos salgados e doces, além de ser um ingrediente popular em receitas de chás e remédios caseiros. O gengibre em pó tem propriedades anti-inflamatórias, antioxidantes e digestivas, além de ser conhecido por ajudar a reduzir náuseas e dores de cabeça. É uma opção prática e versátil para adicionar sabor e benefícios à sua dieta.",
  },
  {
    name: "Gergelim Branco",
    price: 6.0,
    picture: "/assets/products/gergelim-branco.png",
    description:
      "O gergelim branco é uma semente pequena e achatada, com coloração branca ou creme, que pode ser consumida crua ou torrada. É rico em nutrientes como cálcio, ferro, magnésio e fibras. Além disso, é uma fonte de gorduras saudáveis, como os ácidos graxos ômega-6 e ômega-9. É utilizado na culinária para dar sabor e textura a uma variedade de pratos, incluindo saladas, pães e biscoitos.",
  },
  {
    name: "Gergelim Preto",
    price: 6.2,
    picture: "/assets/products/gergelim-preto.png",
    description:
      "O gergelim preto é uma semente oleaginosa e nutritiva, originária da Índia e amplamente utilizada na culinária asiática. Além de seu sabor levemente amargo e textura crocante, o gergelim preto também é conhecido por seus benefícios à saúde, como a presença de antioxidantes, fibras e minerais, incluindo cálcio e ferro. O gergelim preto é frequentemente utilizado para adicionar sabor e textura a pratos como saladas, sopas, arroz e pães, além de ser um ingrediente popular em pastas de gergelim como o tahine.",
  },
  {
    name: "Cravo",
    price: 7.0,
    picture: "/assets/products/cravo.png",
    description:
      "O cravo é uma especiaria conhecida por seu sabor picante e aroma marcante. Ele é comumente utilizado em receitas doces, como em bolos e pudins, mas também pode ser usado em pratos salgados, como carnes e molhos. Além do sabor e aroma, o cravo também é valorizado por suas propriedades medicinais, sendo utilizado para aliviar dores de dente, reduzir a inflamação e tratar problemas respiratórios.",
  },
];
